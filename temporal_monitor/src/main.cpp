...
private:
	...
	// Detected object relate stuff
	geometry_msgs::PointStamped detected_object_pos;
	// Pose
...
public:
	...
	void distanceBetweenPointAndWaypoint(const geometry_msgs::PointStamped& point, const autoware_msgs::Lane& lane, const int& closest_waypoint_id,
			...
			unsigned int& lane_id)
	{
		...
		try
		{
			for (unsigned int i = closest_waypoint_id; i < closest_waypoint_id+50; i++)
			{
...
public:
	...
	void cbDetectedObject(const autoware_msgs::CloudClusterArray::ConstPtr& msg)
	{
		for (const auto& cluster: msg->clusters)
		{
			...
			double longitudinal_distance;
			unsigned int lane_id;
			ego_state_subscriber->distanceBetweenPointAndWaypoint(
					cluster.centroid_point, msg_current_base_lane, closest_waypoint_id,
					longitudinal_distance, lateral_distance, lane_id);
			ROS_DEBUG_STREAM("Closest object distance " << lateral_distance << longitudinal_distance);
			if (lateral_distance * lateral_distance < MAX_LATERAL_DISTANCE_SQR
					&& longitudinal_distance * longitudinal_distance < MAX_LOOKAHEAD_DISTANCE_SQR)
			{
...
object_marker.pose.position.x = point_transformed.point.x;
object_marker.pose.position.y = point_transformed.point.y;
object_marker.pose.position.z = point_transformed.point.z;
